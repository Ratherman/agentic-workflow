...
class Section3Handler(BaseHTTPRequestHandler):
    server_version = "Section3HTTP/0.1"
    ...
    def do_GET(self) -> None:
        ...
        if path == "/health":
            self._set_headers(200)
            self.wfile.write(json.dumps({"ok": True, "section": 3}).encode("utf-8"))
            return
        if path == "/tasks":
            self._set_headers(200)
    ...
    def do_POST(self) -> None:
        path = self.path.split("?", 1)[0].rstrip("/") or "/"
        if path not in {
            "/chat", "/title", "/tasks", "/tasks/toggle", "/tasks/update", "/tasks/edit", "/tasks/delete", "/tasks/remove", "/conversations/sync"
        }:
            self._set_headers(404)
            self.wfile.write(json.dumps({"error": "not found"}).encode("utf-8"))
            return
...
def main() -> None:
    ...
    args = parse_args()
    server = ThreadingHTTPServer(("127.0.0.1", args.port), Section3Handler)
    print(f"Section 3 backend running at http://127.0.0.1:{args.port}")
