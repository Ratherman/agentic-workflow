...
class Section5Handler(BaseHTTPRequestHandler):
    server_version = "Section5HTTP/0.1"
                ...
                json.dumps(
                    {
                        ...
                        "security": {
                            "nemo_guardrails_installed": importlib.util.find_spec("nemoguardrails") is not None,
...
def main() -> None:
    ...
    UPLOADS_DIR.mkdir(parents=True, exist_ok=True)
    server = ThreadingHTTPServer(("127.0.0.1", args.port), Section5Handler)
    print(f"Section 5 backend running at http://127.0.0.1:{args.port}")
