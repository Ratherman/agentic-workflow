...
class Section4Handler(BaseHTTPRequestHandler):
    server_version = "Section4HTTP/0.1"
    ...
    def do_POST(self) -> None:
        path = self.path.split("?", 1)[0].rstrip("/") or "/"
        if path not in {
            "/chat", "/title", "/tasks", "/tasks/toggle", "/tasks/update", "/tasks/edit", "/tasks/delete", "/tasks/remove", "/conversations/sync", "/files"
        }:
            self._set_headers(404)
            self.wfile.write(json.dumps({"error": "not found"}).encode("utf-8"))
            return
...
def main() -> None:
    ...
    UPLOADS_DIR.mkdir(parents=True, exist_ok=True)
    server = ThreadingHTTPServer(("127.0.0.1", args.port), Section4Handler)
    print(f"Section 4 backend running at http://127.0.0.1:{args.port}")
